...
void F88_void_RTC_SetTime(unsigned char ss , unsigned char mm , unsigned char hh)
{
	...
	data_arr[SECONDS] = F90_DecToBcdConvert(ss);
	data_arr[MINUTES] = F90_DecToBcdConvert(mm);
	data_arr[HOURS] = F90_DecToBcdConvert(hh);
...
void F88_void_RTC_GetTime(unsigned char *ss , unsigned char *mm , unsigned char *hh)
{
	...
	*ss = F90_BcdToDecConvert(data_arr[SECONDS]);
	*mm = F90_BcdToDecConvert(data_arr[MINUTES]);
	*hh = F90_BcdToDecConvert(data_arr[HOURS]);
}
